...
resource "aws_cloudfront_distribution" "app" {
  origin {
    ...
    origin_id   = "${var.cf_app_domain_name}_origin"

    s3_origin_config {
      origin_access_identity = "origin-access-identity/cloudfront/${aws_cloudfront_origin_access_identity.app.id}"
    }
  }
  ...
  aliases             = ["${var.cf_app_domain_name}"]
  tags                = "${var.default_tags}"
  ...
  default_cache_behavior {
    ...
    target_origin_id = "${var.cf_app_domain_name}_origin"
...
resource "aws_s3_bucket_policy" "app_allow_cf" {
